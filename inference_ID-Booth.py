...
from diffusers import AutoPipelineForText2Image, DDPMScheduler, DPMSolverMultistepScheduler, StableDiffusionPipeline
...
folder_of_models = "Trained_LoRA_Models"
models_to_test = ["DreamBooth", "PortraitBooth", "ID-Booth"]
checkpoint = "checkpoint-31-6400"

folder_output = "Generated_Samples/FacePortrait_Photo_21"  # _NonFinetuned
if add_gender:
    folder_output += "_Gender"
if add_pose:
    folder_output += "_Pose"
if add_age:
    folder_output += "_Age"
if add_background:
    folder_output += "_Background"
if do_not_use_negative_prompt:
    folder_output += "_NoNegPrompt"
...
negative_prompt = (
    "cartoon, cgi, render, illustration, painting, drawing, black and white, bad body proportions, landscape"
)
original_prompt = "face portrait photo of sks person"
        ...
        for i, num_prompt in enumerate(tqdm(range(num_prompts))):
            ...
            if add_age:
                ...
                if isinstance(prompt_additions, str):
                    age_insert = prompt_additions
                else:
                    age_insert = prompt_additions[0]
                    prompt_additions = prompt_additions[1:]
                if age_insert != "":
                    prompt = prompt.replace(" sks person", f" {age_insert} sks person")

            if add_gender:
                prompt = prompt.replace(" sks person", f" {gender} sks person")
